...
fn test_sql_injection_in_project_name() {
    ...
    let path = test_project
        .create_file("test.py", "print('test')")
        .unwrap();

    let project_id =
        queries::insert_project(&conn, malicious_name, path.to_str().unwrap(), None).unwrap();
...
fn test_unicode_in_project_name() {
    ...
    let project_id =
        queries::insert_project(&conn, unicode_name, path.to_str().unwrap(), None).unwrap();
    let project = queries::select_project(&conn, project_id).unwrap().unwrap();
