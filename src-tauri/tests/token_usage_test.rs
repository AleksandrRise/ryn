...
fn test_cost_analytics_time_range_query() {
    ...
    // Create test project
    let project_id = project
        .insert_project("Analytics Test", "/tmp/test", None)
        .unwrap();
    ...
    let total_input_tokens: i64 = conn
        .query_row("SELECT SUM(input_tokens) FROM scan_costs", [], |row| {
            row.get(0)
        })
        .unwrap();

    let total_output_tokens: i64 = conn
        .query_row("SELECT SUM(output_tokens) FROM scan_costs", [], |row| {
            row.get(0)
        })
        .unwrap();

    let total_cache_read: i64 = conn
        .query_row("SELECT SUM(cache_read_tokens) FROM scan_costs", [], |row| {
            row.get(0)
        })
        .unwrap();
    ...
    assert_eq!(
        total_cache_read, 90_000,
        "Cache read tokens should sum correctly"
    );
    assert_eq!(
        total_cache_write, 22_500,
        "Cache write tokens should sum correctly"
    );
    ...
    assert!(
        costs_desc[0] > costs_desc[1],
        "Costs should be in descending order"
    );
    assert!(
        costs_desc[1] > costs_desc[2],
        "Costs should be in descending order"
    );
...
fn test_scan_cost_cascade_delete() {
    ...
    let exists_before: bool = project
        .connection()
        .query_row(
    ...
    assert!(
        !exists_after,
        "Scan cost should be cascade deleted when scan is deleted"
    );
}
...
fn test_zero_files_cost_tracking() {
    ...
    let (files, cost): (i64, f64) = project
        .connection()
        .query_row(
    ...
    assert_eq!(
        scan_cost.cost_per_file(),
        0.0,
        "Cost per file should be 0 when no files"
    );
    assert_eq!(
        scan_cost.avg_tokens_per_file(),
        0.0,
        "Avg tokens per file should be 0 when no files"
    );
}
