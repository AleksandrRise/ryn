  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => async (state) => {
        ...
        // If no project is selected after hydration, try to default
        // to the first available project (useful for web demo mode).
        try {
          const projects = await get_projects()
          if (projects.length > 0) {
            const first = projects[0]
            console.log('[project-store] No project in storage, defaulting to project:', first.id)
            state.setSelectedProject(first)
          }
        } catch (error) {
          console.warn('[project-store] Failed to load default project:', error)
        }
      },
    }
  )
